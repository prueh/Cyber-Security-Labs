# Cyber-Security-Labs
Technical documentation and detection engineering projects from my Cybersecurity SOC Internship, covering SIEM configuration, NIDS, and Malware Analysis
Welcome to my technical portfolio. This repository contains the weekly progress, technical configurations, and detection engineering reports from my SOC Internship.

🛠 Tech Stack
* **SIEM:** Wazuh
* **NIDS:** Suricata
* **Firewall/Perimeter:** pfSense
* **Operating Systems:** Ubuntu Server, Windows 10, Kali Linux
* **Frameworks:** MITRE ATT&CK, NIST CSF

📂 Project Structure
* **[Week 2: SIEM Foundations](./Week-2-SIEM-Foundations)** - FIM, Custom Rules, and Active Response.
* **[Week 3: NIDS Integration](./Week-3-NIDS-Integration)** - Suricata deployment and packet inspection.
* **[Week 4: Perimeter & Vulnerabilities](./Week-4-Perimeter-Defense)** - pfSense routing and automated patching.
* **[Week 5: Malware Analysis](./Week-5-Malware-Analysis)** - njRAT and WannaCry detonation & signature writing.
* **[Week 6: Data Exfiltration Mitigation](./Week-6-Exfiltration-Mitigation)** - Custom alerting for T1027 obfuscation using certutil.
